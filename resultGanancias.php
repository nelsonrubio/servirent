...
$gasto = mysqli_query($con, "select * from cajachica where  fecha = '$_REQUEST[fecha]'") or
die("Problemas en el select:" . mysqli_error($con));
?>
<!DOCTYPE html>
<html lang="en">
                  ...
                  <tbody>
                    ...
                    $totalGasto = 0;
                        while ($reg = mysqli_fetch_array($usuario)) {
                          $idPago = $reg['metodoPago'];
                          $idPedido3 = $reg['idPedido'];
                          $total += $reg['total'];
                          $metodo = mysqli_query($con, "select * from metodospago where idPago = $idPago") or
                          die("Problemas en el select:" . mysqli_error($con));
                          $reg3 = mysqli_fetch_array($metodo);
