...
if(isset($_POST['submit'])){
    ...
    if(!empty($nameArr)){
        ...
        values ('$_POST[usuario]','$tlf',$bodega,$chofer,$pago,'$newFecha',$estatus,'$direccion',$total,$cupon, $descuento, $totalCilindro, '$horaPedido',1)") or 
        die("Problemas en el select" . mysqli_error($con));
        if(mysqli_affected_rows($con)){
            ...
            for($i = 0; $i < count($nameArr); $i++){
                if(!empty($nameArr[$i])){
                    ...
                    die("Problemas en el select" . mysqli_error($con));
                    echo '<script> alert("Registro completado, por favor acepte para continuar..."); window.location = "tomarPedido.php";</script>';
                    //database insert query goes here
                }
            }
